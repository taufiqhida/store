  ...
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal admin-modal">
      <div class="modal-header">
        <h3>{{ editing ? 'Edit' : 'Tambah' }} Kategori</h3>
        <button class="modal-close" @click="$emit('close')">×</button>
      </div>
      <div class="modal-body">
        <div class="form-group">
          <label>Nama Kategori</label>
          <input v-model="form.name" @input="generateSlug" placeholder="Nama kategori" />
        </div>
        <div class="form-group">
          <label>Slug</label>
          <input v-model="form.slug" placeholder="slug-kategori" />
        </div>
      </div>
...
.modal { background: white; border-radius: 16px; max-width: 450px; width: 100%; }
